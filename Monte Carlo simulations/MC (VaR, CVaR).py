...
def mcVar (returns, alpha=5):
    if isinstance(returns, pd.Series):#if returns is a series of data do the following
        return numpy.percentile(returns, alpha)# spits the number for which 5% of numbers are below it.
    
def mcCVar (returns, alpha=5):
    if isinstance(returns, pd.Series):
        belowVaR = returns <= mcVar(returns, alpha=5) #you wanna look at the returns bewlow that 5th percentile which is a subset of the retruns series which is wha the <= operator does
        return returns[belowVaR].mean() #still have to access them through returns + spit the meam value of those belowVar values

portResults=pd.Series(portfolio_sims[-1,:]) #the series you are looking for is the 100th day results so take the last row of the simulations sorted in portfolio_sims that is [-1,:] remmember the first row is [0,:] idenxisng starts at 0

VaR= initialPortfolio - mcVar(portResults, alpha=5) #the RHS is one simulation result a rumber on that y axis for days = 100 (not how much you lost but how much you are expected to have which results in a loss if you thake the difference from what you originally had)
CVaR= initialPortfolio - mcCVar(portResults, alpha=5)# same here
